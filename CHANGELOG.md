...
- 修复闲鱼客户图片仅以 `[图片]` 文字显示：按源消息 ID 补拉单张图片资源，经格式和大小校验后上传飞书并引用原客户卡片发送，保留固定回复路由与失败时的文字提醒。
- 将整个仓库统一为根 Skill（布局版本 0.2.0）：根 SKILL.md 与唯一 src，统一管理脚本和公开资源清单，支持已有项目直接管理、白名单导出及单份 runtime 部署；原业务包版本和消息处理逻辑不变。
- 新增 xianyu-feishu-bridge 0.1.0 自包含 Skill、资源清单和 NPX 复制安装验证，以及独立实例初始化、结构化诊断、进程身份/心跳、启停、SQLite 备份升级和当前用户自启管理。原消息桥和客户路由保留，生产实例未迁移或重启；发布门槛见 docs/skill-validation.md。
- 增加飞书直接引用的单张 PNG/JPG/JPEG 图片回复：资源下载、实际格式校验、目标闲鱼账号上传、原连接发送和失败原因提示；回复任务数据库非破坏性升级到 v4，保留文本路径、去重与未知结果不重发规则。重启后用户已确认图片发送测试成功。
- 补充 README 原仓库致谢及完整使用说明，覆盖环境安装、飞书配置、两种账号接入方式、绑定、回复、启停、自启和排障；同步启动文档中的动态账号数量、窗口匹配方式及当前发送限流说明。
